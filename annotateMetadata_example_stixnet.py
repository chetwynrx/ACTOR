...
tagger.model=Ollama(id=user_defined_model, options={"num_ctx":3000, "temperature":0.1}, keep_alive=0)
reviewer.model=Ollama(id=user_defined_model, options={"num_ctx":3000, "temperature":0.02}, keep_alive=0)
...
def getAnnotation(indexValue):
    labelList = []
    with open('./datasets/inputs/STIXnet/annotations/Annotations.json', 'r', encoding='utf-8') as file:
    
        data = json.load(file)
        
            
        excerpt = data[indexValue]["data"]["text"]
            
        annotations = data[indexValue]["annotations"]
        
            # For each annotation
    for a in annotations:
    
        # The individual annotations generated by the authors of STIXnet are in a list
        results = a["result"]
        
        # For each annotation in the dataset we look for those with type labels
        for result in results:
            if "labels" in result["type"]:
                #print("True")
...
if __name__ == "__main__":      
    for documentNumber in range(0,53):
        print(f"\n{'='*60}")
        print(f"Processing document {documentNumber}")
        print(f"{'='*60}\n")
        excerpt = getExcerpt(documentNumber)
        current_doc_number = documentNumber

        # Create and use workflow
    
        metadata_annotation_workflow.print_response(
            input=fr"The document text to process is: '{getExcerpt(documentNumber)}', For the JSON output: The document ID is: {documentNumber}, the page number is: {documentNumber}. The title is: '{documentNumber}'. Include the full document text in the JSON output.",
            markdown=True,
            stream=True, # enable streaming
            stream_events=True # stream workflow events
        )
        
        # Have to set a rate limit 
        print(f"\n{'='*60}")
        print(f"Setting a 2 second delay")
        print(f"{'='*60}\n")
        time.sleep(3)
